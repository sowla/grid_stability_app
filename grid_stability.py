...
### side bar ###
def add_input_set(feature: str, min_value: float, max_value: float, value: float, node_names: List[str]):
    inputs = [st.slider(node_name, min_value, max_value, value) for node_name in node_names]
...
with st.sidebar:
    ...
    with st.beta_expander("Response delay", expanded=False):
        st.write("How long it takes for each node to adapt their production or consumption in seconds:")
        p_delay_df = add_input_set("p_delay", 0.5, 10.0, 0.5, ["Producer"])
        c_delay_df = add_input_set("c_delay", 0.5, 10.0, 5.0, ["Consumer1", "Consumer2", "Consumer3"])

    with st.beta_expander("Willingness to adapt", expanded=True):
        st.write("Willingness of each node to adapt their consumption or production per second:")
        p_adapt_df = add_input_set("p_adapt", 0.05, 1.0, 0.05, ["Producer"])
        c_adapt_df = add_input_set("c_adapt", 0.05, 1.0, 0.5, ["Consumer1", "Consumer2", "Consumer3"])
...
st.markdown(f"Best regressor's prediction: {round(reg_pred, 3)} (**{stability_dict.get(int(reg_pred < 0))}**)")
